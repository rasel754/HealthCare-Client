  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Doctor Medical Specialties Mapping</h1>
          <p className="text-xs text-slate-500 mt-1">Review specialty tags assigned to registered doctors</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="py-12 text-center text-xs text-slate-500">Loading doctor specialties mapping...</div>
      ) : doctors.length === 0 ? (
        <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center space-y-3 shadow-xs">
          <Layers className="h-12 w-12 text-slate-300 mx-auto" />
          <h3 className="text-base font-bold text-slate-800">No Specialty Mappings Found</h3>
          <p className="text-xs text-slate-400">Doctor specialty assignments will appear here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {doctors.map((doc) => {
            ...
            return (
              <div key={doc.id} className="bg-white rounded-3xl border border-slate-200 p-6 space-y-4 shadow-xs">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-emerald-700 font-medium">{doc.designation}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100">
                  ...
                  {specialtiesList.length === 0 ? (
                    <p className="text-xs text-slate-400 italic">No specialties assigned.</p>
                  ) : (
