  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Doctor Assigned Schedules</h1>
          <p className="text-xs text-slate-500 mt-1">Overview of time slots claimed by specialist doctors</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="py-12 text-center text-xs text-slate-500">Loading doctor schedules...</div>
      ) : schedules.length === 0 ? (
        <div className="bg-white p-12 rounded-3xl border border-slate-200 text-center space-y-3 shadow-xs">
          <Clock className="h-12 w-12 text-slate-300 mx-auto" />
          <h3 className="text-base font-bold text-slate-800">No Doctor Schedules Found</h3>
          <p className="text-xs text-slate-400">Time slot assignments will appear when doctors select master schedules.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {schedules.map((ds) => (
            <div key={`${ds.doctorId}-${ds.scheduleId}`} className="bg-white rounded-3xl border border-slate-200 p-6 space-y-4 shadow-xs">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-slate-900 text-base">{ds.doctor?.name || "Doctor"}</h3>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl space-y-2 text-xs text-slate-600">
                ...
                <div className="pt-2 border-t border-slate-200 flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Booking Status</span>
                  <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${ds.isBooked ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}>
