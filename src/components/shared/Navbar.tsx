      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
            ...
            {user ? (
              <div className="flex items-center gap-3">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="gap-2 text-destructive hover:bg-destructive/10 hover:text-destructive rounded-xl cursor-pointer"
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-b border-border bg-background px-4 pt-2 pb-6 space-y-3">
            {user && (
              <div className="flex items-center gap-3 p-3 rounded-2xl bg-muted/60 border border-border">
                ...
                <div className="overflow-hidden">
                  <p className="font-semibold text-sm text-foreground truncate">
                    {user.name || user.patient?.name || user.doctor?.name || user.admin?.name || "User"}
                  </p>
                  <p className="text-xs text-primary font-bold uppercase tracking-wider">{user.role || "PATIENT"}</p>
            ...
            <div className="pt-4 border-t border-border flex flex-col gap-2">
              {user ? (
                <>
                  ...
                  <Button
                    variant="destructive"
                    className="w-full justify-start gap-2 rounded-xl cursor-pointer"
                    onClick={() => {
